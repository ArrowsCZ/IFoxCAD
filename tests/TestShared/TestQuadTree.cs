...
public partial class TestQuadTree
{
        ...
        Tools.TestTimes(
            ...
            () =>
            {
                ...
                foreach (var ent in grc)
                {
                    ...
                    var ce = new CadEntity(entId, entRect) { Color = RandomEx.NextColor() };
                    ces.Add(ce);
                    /*加入随机点*/
    ...
    void AddQuadTreeRoot(Database db, Editor ed, List<ObjectId> ss)
    {
        if (_quadTreeRoot is null)
        {
            ...
            {
                dbExt = new Rect(
                    dbExtent.Value.MinPoint.Point2d(),
                    dbExtent.Value.MaxPoint.Point2d()
                );
            }
        ...
        db.Action(tr =>
        {
            ss.ForEach(entId =>
            {
                ...
                // 四叉树数据
                var ce = new CadEntity(entId, edge.Edge) { Color = Utility.RandomColor };
                ces.Add(ce);
    ...
    public static Rect? GetCorner(Editor ed)
    {
        ...
        return new Rect(
            new Point2d(pprA.Value.X, pprA.Value.Y),
