...
public class TestExtents
{
    ...
    [CommandMethod(nameof(Test_Extensions))]
    public void Test_Extensions()
    {
        using var tr = new DBTrans();
        var a = Env.Editor.GetSelection().Value.GetEntities<Entity>(OpenMode.ForWrite);
        foreach (var e in a)
        {
            ...
            if (b)
            {
                tr.CurrentSpace.AddEntity(
                    e.Bounds!.Value.CreatePolyline(action: e =>
                    {
                        e.ColorIndex = 4;
                        e.Closed = true;
                    })
